...
void minMax(std::condition_variable & cv, std::atomic<bool> & flag, const Board board, std::mutex & critical, int & move) {
	...
	for (int i=0; i <= 100000; ++i) {
		...
		try {
			// move down right
			for (int j = 9 + board.getComputer(); j%8 > board.getComputer()%8 && j < 63; j+=9) {
				if(board.test(j)) {
					break;
				}
				Board next = board;
			...
			// move down left
			for (int j = 7 + board.getComputer(); j%8 < board.getComputer() %8  && j < 63; j+=7) {
				if(board.test(j)) {
					break;
				}
				Board next  = board;
			...
			// move down
			for (int j = 8 + board.getComputer(); j < 63; j+=8) {
				if(board.test(j)) {
					break;
				}
				Board next = board;
...
int algoMin(std::atomic<bool> & flag, const int depth, int alpha, int beta, Board board) {
	...
	try {
		...
		// move down right
		for (int j = 9 + board.getOpponent(); j%8 > board.getOpponent()%8 && j < 63; j+=9) {
			if(board.test(j)) {
				break;
			}
			Board next = board;
		...
		// move down left
		for (int j = 7 + board.getOpponent(); j%8 < board.getOpponent()%8  && j < 63; j+=7) {
			if(board.test(j)) {
				break;
			}
			Board next  = board;
		...
		// move down
		for (int j = 8 + board.getOpponent(); j < 63; j+=8) {
			if(board.test(j)) {
				break;
			}
			Board next = board;
