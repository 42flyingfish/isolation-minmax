...
int BucketAgent::getAiTurn(const Board board) {
...
int BucketAgent::wrapper(const Board board) {
	std::mutex m, critical;
	std::condition_variable cv;
	std::atomic<bool> flag {false};
	int move;

	std::thread third(&BucketAgent::minMax, this, std::ref(cv), std::ref(flag), board, std::ref(critical), std::ref(move));
	third.detach();
...
void BucketAgent::minMax(std::condition_variable & cv, std::atomic<bool> & flag, const Board board, std::mutex & critical, int & move) {
...
int BucketAgent::algoMin(std::atomic<bool> & flag, const int depth, int alpha, int beta, Board board) {
	if (depth == 0 ) {
...
int BucketAgent::algoMax(std::atomic<bool> & flag, const int depth, int alpha, int beta, Board board) {
	if (depth == 0 ) {
		return evaluate(board);
	}
	if (isTerminal(board)) {
...
int BucketAgent::evaluate(const Board board) {
...
int BucketAgent::count(const Board board, const int playerPosition) {
	int localSum = 0;
	// move down right
...
bool BucketAgent::isTerminal(const Board board) {
	if (terminalMap.find(board.getBitset()) != terminalMap.end()) {
		return terminalMap[board.getBitset()];
	}
	bool value = board.gameOver();
	terminalMap[board.getBitset()] = value;
	return value;
}
