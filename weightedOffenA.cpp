...
int WeightedOffenA::getAiTurn(const Board board) {
...
int WeightedOffenA::wrapper(const Board board) {
	std::mutex m, critical;
	std::condition_variable cv;
	std::atomic<bool> flag {false};
	int move;

	std::thread third(&WeightedOffenA::minMax, this, std::ref(cv), std::ref(flag), board, std::ref(critical), std::ref(move));
	third.detach();
...
void WeightedOffenA::minMax(std::condition_variable & cv, std::atomic<bool> & flag, const Board board, std::mutex & critical, int & move) {
...
int WeightedOffenA::algoMin(std::atomic<bool> & flag, const int depth, int alpha, int beta, Board board) {
	if (depth == 0 ) {
...
int WeightedOffenA::algoMax(std::atomic<bool> & flag, const int depth, int alpha, int beta, Board board) {
	if (depth == 0 ) {
...
int WeightedOffenA::count(const Board board, const int playerPosition) const {
	int localSum = 0;
	// move down right
